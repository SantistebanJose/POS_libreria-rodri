            ...
            <div class="card-body">
                ...
                <div class="table-responsive">
                    <table
                        ...
                        <tbody>
                            <?php
                            foreach (fnListForClientesDeuda() as $datos) {
...
<script>
    function fnAbonarDeuda() {
        ...
        var montosAcumn = 0;
        document.querySelectorAll('[id^="pagoCredito_"]').forEach(function(pago) {
            var formaPago = pago.querySelector('select').value;
            var monto = pago.querySelector('input').value;
            monto = parseFloat(monto); // Convertir a número decimal

            if (formaPago && !isNaN(monto)) {
                montosAcumn += monto; // Sumar el monto al total acumulado
                datosPagos.push({
                    id_forma_pago: parseInt(formaPago), // Convertir el valor del select a número
        ...
        var idCliente = document.getElementById("idDeudorDeMrd").textContent;
        console.log("DEUDOR ID DE MRD: ", idCliente);
        console.log("ID USUARIO EN SESSION: ", <?php echo $id_usuario_s ?>)
        console.log("MONTO DEUDA GENERAL: ", montoDeudaGenral);
        console.log("DATOS DE LOS PAGOS ", datosPagos);
        if (montosAcumn == 0) {
            swal("Ups!, No ingresaste ningún monto", {
                icon: "error",
                buttons: {
                    confirm: {
        ...
        } else if (montosAcumn > montoDeudaGenral) {
            swal("Ups!, Los montos ingresados son MAYORES al Monto final de la Deuda 😅", "Agrega correctamente los montos :)", {
                icon: "error",
                buttons: {
                    confirm: {
                        className: "btn btn-danger",
            ...
            var js_abono = {
                ...
                "usuario_id": <?php echo $id_usuario_s ?>,
                "montoAbono": montosAcumn,
                "js_detalle_pagos": datosPagos
            };
            $.ajax({
                url: 'logica/clssInsertPA.php',
                type: 'POST',
                data: {
                    accion: 'ABONARDEUDACLIENTE',
                    jsDatosAbono: JSON.stringify(js_abono)
                },
...
<script>
    function fnAbrirModalPagarCredito(jsonDatosCliente) {
        //idNombreDeudor
        fn_limpiarTabla();
        document.getElementById("idDeudorDeMrd").innerText = jsonDatosCliente.cliente_id;
